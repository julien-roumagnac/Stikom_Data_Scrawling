...
def createDatabase():
    for db in engines:
        ...
        db.execute("CREATE TABLE user_newsletter (id varchar(50) NOT NULL, username varchar(100) NOT NULL,phonenumber varchar(50), email varchar(100) NOT NULL, prefbroadcast varchar(5), sex varchar(2), company varchar(100), home_address varchar(500),occupation varchar(100), knowing_bps_from varchar(100),survey_respondent varchar(100))")
        db.execute("CREATE TABLE flags (id varchar(50) NOT NULL, user_id varchar(100) NOT NULL ,news_id varchar(100) NOT NULL)")

        userArray = np.array([[1, 'TestUser', '+628123456789', 'gianyarkab.newsletter@gmail.com', '001', '0', 'CompanyXXX', 'Stikom bali denpasar 5432 oo Baz','Student','Social Media','1']])
        userDf = pandas.DataFrame(userArray,
                                  columns=['id', 'username', 'phonenumber', 'email', 'prefbroadcast', 'sex',
                                           'company', 'home_address','occupation','knowing_bps_from','survey_respondent'])
